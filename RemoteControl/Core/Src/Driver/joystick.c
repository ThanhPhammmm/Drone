...
#define STICK_DEADBAND       25        /* raw ADC counts around center to ignore */

/* hadc1 itself is declared in CubeMX's generated adc.h, included above. */

static volatile uint16_t adcRaw[JOY_NUM_CHANNELS];
static uint16_t stickCenter[JOY_NUM_CHANNELS];   /* roll/pitch/yaw only */

/* Averages a few DMA-filled snapshots. Called once at boot with the sticks
 * assumed centered (throttle at idle, roll/pitch/yaw centered) -- that's the
 * standard "power on with sticks at rest" convention, same as any hobby TX. */
...
void JOY_Init(void){
    /* ADC1 pins (PA0/PA1/PB0/PB1 -> analog) and the arm switch pin
     * (PA2 -> input, pull-up) are configured by CubeMX's MX_GPIO_Init /
     * MX_ADC1_Init, called before this task starts -- nothing to set up
     * here beyond starting the DMA stream and taking the boot calibration. */
...
void JOY_Read(RC_Packet_t *pkt){
    ...
    pkt->flags = (HAL_GPIO_ReadPin(ARM_SW_PORT, ARM_SW_PIN) == GPIO_PIN_RESET)
                     ? RC_FLAG_ARM : 0;
}
